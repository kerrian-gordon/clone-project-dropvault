...
export function StorageMeter({ usage, error }) {
  ...
  return <div className={`storage-meter${ratio >= WARN_THRESHOLD ? ' warn' : ''}`}>
    ...
    <p>{formatBytes(usage.usedBytes)} of {formatBytes(usage.limitBytes)} used · {usage.tier} plan
      {ratio >= 1 ? <strong> · Storage full</strong> : ratio >= WARN_THRESHOLD ? <strong> · Almost full</strong> : null}
    </p>
  </div>;
}
