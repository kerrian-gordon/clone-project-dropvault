...
- Registration is limited to 10 attempts per source IP and login to 5 failed attempts per source IP and email (also 30 across emails) within 15 minutes. These counters live in one API process and reset on restart; production hosting needs shared, proxy-aware throttling.
...
- File deletion stages the bytes in `storage/tmp/` while metadata is removed. The API waits for active downloads, and startup recovery restores a staged file whose catalog record still exists or removes staged bytes after a committed deletion. Older temporary files made before this recovery format are left untouched for manual inspection.
