...
This document defines the multi-user freemium milestone for Dropvault. The local API includes sessions, ownership, per-user quotas, named-user access, and a demo tier switch. The browser includes account entry, owned and shared listings, per-file uploads, deletion, access controls, a storage meter, and a demo upgrade prompt that retries a blocked file. Inline previews, bearer-link sharing UI, and a full browser integration check remain to be built. See [the implemented API contract](api.md) for exact routes and current limits.
